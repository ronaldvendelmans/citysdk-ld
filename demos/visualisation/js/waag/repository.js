...
var apiUrlNL="http://api.citysdk.waag.org/";
var apiUrlIstanbul="http://apicitysdk.ibb.gov.tr/"
var apiUrlLamia="http://api.citysdk.lamia-city.gr/";
...
var llIstanbul = {
	city: "istanbul",
	apiUrl:apiUrlIstanbul,
	admr: "admr.istb.uskudar",
	lat: 41.0128,
	lng: 28.9744,
	d: 50
}; // istanbul
...
var llHelsinki = {
	city: "helsinki",
	apiUrl:apiUrlNL,
	admr: "admr.fi.helsinki",
	lat: 60.2311,
	lng: 24.9898,
	d: 20
}; // helsinki 60.23117077837895, 24.989808077582467
var llTampere = {
	city: "tampere",
	apiUrl:apiUrlNL,
	admr: "admr.fi.tampere",
	lat: 61.4943,
	lng: 23.7966,
	d: 20
}; // Tampere
...
function initRepository() {
	...
	dataLayer = {
		...
		layers: [{
			label: "Amsterdam",
			subs: ["tram", "bus", "subway", "ferry"],
			ll: llAmsterdam,
			apiCall: apiUrlNL + llAmsterdam.admr + "/ptlines?geom",
			regions: false,
			layer: "amsterdam_pt_live",
			properties: {
				static: false,
				dotSize: 0.25,
				lines: true,
			...
			subs: ["tram", "bus", "subway", "ferry"],
			ll: llUtrecht,
			apiCall: apiUrlNL +llUtrecht.admr + "/ptlines?geom",
			regions: false,
			layer: "utrecht_pt_live",
			properties: {
				static: false,
				dotSize: 0.25,
				lines: true,
			...
			subs: ["tram", "bus", "subway", "ferry"],
			ll: llDenHaag,
			apiCall: apiUrlNL + llDenHaag.admr + "/ptlines?geom",
			regions: false,
			layer: "sgravenhage_pt_live",
			properties: {
				static: false,
				dotSize: 0.25,
				lines: true,
			...
			subs: ["tram", "bus", "subway", "ferry"],
			ll: llRotterdam,
			apiCall: apiUrlNL + llRotterdam.admr + "/ptlines?geom",
			regions: false,
			layer: "rotterdam_pt_live",
			properties: {
				static: false,
				dotSize: 0.25,
				lines: true,
		...
		//{label:"Eindhoven", subs:["tram", "bus", "subway", "ferry"], ll:llEindhoven, apiCall:llRotterdam.admr+"/ptlines?geom", regions:false, layer:"eindhoven_pt_live", properties:{static:false, dotSize:0.25, lines:true, shape:false, stacked:false}},
		{
			...
			subs: ["tram", "bus", "subway", "ferry"],
			ll: llTampere,
			apiCall: apiUrlNL + llTampere.admr + "/ptlines?geom",
			regions: false,
			layer: "tampere_pt_live",
			properties: {
				static: false,
				dotSize: 0.25,
				lines: true,
			...
			subs: ["tram", "bus", "subway", "ferry"],
			ll: llHelsinki,
			apiCall: apiUrlNL + llHelsinki.admr + "/ptlines?geom",
			regions: false,
			layer: "helsinki_pt_live",
			properties: {
				static: false,
				dotSize: 0.25,
				lines: true,
			...
			subs: ["tram", "bus", "subway", "ferry"],
			ll: llManchester,
			apiCall: apiUrlNL + llManchester.admr + "/ptlines?geom",
			regions: false,
			layer: "manchester_pt_live",
			properties: {
				static: false,
				dotSize: 0.1,
				lines: true,
			...
			subs: ["tram", "bus", "subway", "ferry"],
			ll: llIstanbul,
			apiCall: apiUrlIstanbul + llIstanbul.admr + "/ptlines?geom",
			regions: false,
			layer: "istanbul_pt_live",
			properties: {
				static: false,
				dotSize: 0.25,
				lines: true,
  			...
  			subs: ["tram", "bus", "subway", "ferry"],
  			ll: llLamia,
  			apiCall: apiUrlLamia + llLamia.admr + "/ptlines?geom",
  			regions: false,
  			layer: "lamia_pt_live",
  			properties: {
  				static: false,
	...
	dataLayer = {
		...
		layers: [{
			...
			subs: [],
			ll: llAmsterdam,
			apiCall: apiUrlNL + "nodes?layer=divv.traffic&geom",
			regions: false,
			layer: "trafic_flow_adam",
			properties: {
				static: false,
				dotSize: 0.5,
				lines: true,
	...
	dataLayer = {
		...
		layers: [{
			...
			subs: [],
			ll: llNl,
			apiCall: apiUrlNL + "nodes?layer=artsholland&geom",
			regions: false,
			layer: "artsholland",
			properties: {
				static: false,
	...
	dataLayer = {
		...
		//{label:"Manchester", subs:[], ll:llManchester, apiCall:llManchester.admr+"/ptlines?geom", regions:false, layer:"manchester_ptlines", properties:{static:true, dotSize:0.25, lines:true, shape:false, stacked:false}},
		{
			label: "Helsinki",
			subs: ['lines', 'stops'],
			ll: llHelsinki,
		 ...
		 {
			label: "Netherlands (all stops)",
			subs: [],
			ll: llNl,
			apiCall: apiUrlNL + llNl.admr + "/regions?admr::admn_level=3",
			regions: true,
			layer: "nl_ptstops",
			properties: {
				static: true,
	...
	// Bag http://api.citysdk.waag.org/admr.nl.nederland/nodes?layer=bag.ligplaatsen&geom
	// dataLayer={
	// 		label:"BAG",
	// 		layers:[
	// 			{label:"Housebouts NL", subs:[], ll:llNl, apiCall:llNl.admr+"/regions?admr::admn_level=3", regions:true, layer:"houseboats_nl", properties:{static:true, dotSize:0, lines:false, shape:true, stacked:true}}
	// 		]
	// 	};
	// 	
	// dataLayers.push(dataLayer);
  ...
  //  layers: [{
  //    label: "U.S. Drone attacks",
  //    subs: [],
  //    ll: llDroneAttacks,
  //    apiCall: "http://api.dronestre.am/data",
  //    regions: false,
  //    layer: "drone_attacks",
  //    properties: {
  //      static: true,
...
function getApiData(dataLayer) {
	...
	if (dataLayer.regions) {
		getRegions(dataLayer.apiCall + "&per_page=" + maxEntrys + "&page=1", 1, dataLayer, null);
	} else {
	  addloadDataQueue(dataLayer.apiCall+"&per_page="+maxEntrys+"&page=1", 1, dataLayer, null);
	  
    // if(dataLayer.label!="Lamia"){
    //   addloadDataQueue(apiUrl+dataLayer.apiCall+"&per_page="+maxEntrys+"&page=1", 1, dataLayer, null);
    // }else if(dataLayer.label=="Lamia"){
    //   var uriLamia="http://api.citysdk.lamia-city.gr/"+dataLayer.apiCall+"&per_page="+maxEntrys+"&page=1";
    //   addloadDataQueue(uriLamia, 1, dataLayer, null);
    // };
	}
...
function getRegions(uri, page, dataLayer, resultsArray) {
	...
	d3.json(uri, function(data) {
		...
		for (var i = 0; i < data.results.length; i++) {
			...
			} else if (layerOrg == "houseboats_nl") {
				dataLayer.layer = "houseboats_nl";
				// Bag http://api.citysdk.waag.org/admr.nl.nederland/nodes?layer=bag.ligplaatsen&geom
				url = apiUrl + id + "/nodes?layer=bag.ligplaatsen&geom&per_page=" + maxEntrys + "&page=1";
...
function getPtLinesData(dataLayer) {
	...
	//var url=dataLayer.apiUrl;
	$("#feedback").text("getPtLinesData : " + dataLayer.apiCall );
	$.getJSON( dataLayer.apiCall + "&per_page=" + maxEntrys + "&page=1", function(data) {
		var ptLine = {
		...
		for (var i = 0; i < ptLine.lines.length; i++) {
			...
			var uri = dataLayer.ll.apiUrl + cdk_id + "/select/schedule";
			getSchedules(uri, i, dataLayer.layer, ptSchedules.length - 1);

			var uri = dataLayer.ll.apiUrl + cdk_id + "/select/nodes?per_page=100";
			getNodeInfo(uri, i, dataLayer.layer, ptSchedules.length - 1);
