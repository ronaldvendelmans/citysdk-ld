...
tile_cmd = "#{tilemill_path}/index.js export bag #{export_path}/%s.%s --format=%s --width=%s --height=%s --bbox=%s --files=#{projects_path}"
...
(1..7).each do |page|
  ...
  results.each do |gemeente|
    name = gemeente["name"]
    filename = name.downcase.to_ascii.gsub(/[^a-z]/, "_")
    geom = RGeo::GeoJSON.decode(gemeente["geom"].to_json, :json_parser => :json)
...
File.open("gemeentes.json", 'w') { |file| file.write(JSON.pretty_generate(gemeentes)) }
puts "Done..."
